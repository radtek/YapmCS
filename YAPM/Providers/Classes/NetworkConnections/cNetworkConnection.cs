...
public class cNetworkConnection : cGeneralConnection
{
    internal static int instanceId = 1;
    private int _instanceId = 1;
    private asyncCallbackNetworkEnumerate _networkEnum;

    public cNetworkConnection(Control ControlWhichGetInvoked, ref cConnection Conn, ref HasEnumeratedEventHandler de) : base(ControlWhichGetInvoked, ref Conn)
    {
        instanceId += 1;
        _instanceId = instanceId;
        _networkEnum = new asyncCallbackNetworkEnumerate(ref _control, de, ref this, _instanceId);
    }
    ...
    public delegate void HasEnumeratedEventHandler(bool Success, Dictionary<string, networkInfos> Dico, string errorMessage, int instanceId);
    ...
    // Enumerate services
    public int Enumerate(bool getFixedInfos, ref int pid, bool all, int forInstanceId = -1)
    {
        System.Threading.ThreadPool.QueueUserWorkItem(new System.Threading.WaitCallback(_networkEnum.Process()), new asyncCallbackNetworkEnumerate.poolObj(pid, all, forInstanceId));
    }
    ...
    protected new void _sock_ReceivedData(ref cSocketData data)
    {
        ...
        if (data.Type == cSocketData.DataType.RequestedList && data.Order == cSocketData.OrderType.RequestNetworkConnectionList)
        {
            if (_instanceId == data.InstanceId)
                // OK it is for me
                _networkEnum.GotListFromSocket(ref data.GetList, ref data.GetKeys);
        }
    }

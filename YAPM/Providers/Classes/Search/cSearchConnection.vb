...
Public Class cSearchConnection
    ...
    Public Sub New(ByVal ControlWhichGetInvoked As Control, ByRef Conn As cConnection, ByRef de As HasEnumeratedEventHandler)
        ...
        _searchEnum = New asyncCallbackSearchEnumerate(_control, de, Me, _instanceId)
    End Sub
...
#Region "Events, delegate, invoke..."
    ...
    Public Delegate Sub HasEnumeratedEventHandler(ByVal Success As Boolean, ByVal Dico As Dictionary(Of String, searchInfos), ByVal errorMessage As String, ByVal instanceId As Integer)
    ...
    Protected Overrides Sub asyncDisconnect(ByVal useless As Object)
        Select Case _conObj.ConnectionType
            ...
            Case Else
                ' Local
                _connected = False
                If Disconnected IsNot Nothing AndAlso _control.Created Then
                    _control.Invoke(Disconnected, True)
                End If
        End Select
    End Sub
...
#Region "Enumerate services"
    ...
    Public Function Enumerate(ByRef str As String, ByVal caseS As Boolean, ByVal include As Native.Api.Enums.GeneralObjectType, Optional ByVal forInstanceId As Integer = -1) As Integer
        Call Threading.ThreadPool.QueueUserWorkItem(New  _
                ...
                asyncCallbackSearchEnumerate.poolObj(str, caseS, include, forInstanceId))
    End Function
...
#Region "Sock events"
    ...
    Protected Shadows Sub _sock_ReceivedData(ByRef data As cSocketData) Handles _sock.ReceivedData
        ...
        If data.Type = cSocketData.DataType.RequestedList AndAlso _
            data.Order = cSocketData.OrderType.RequestSearchList Then
            If _instanceId = data.InstanceId Then
                ' OK it is for me
                _searchEnum.GotListFromSocket(data.GetList, data.GetKeys)
            End If
        End If
    End Sub

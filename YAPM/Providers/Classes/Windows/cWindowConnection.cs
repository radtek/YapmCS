...
public class cWindowConnection : cGeneralConnection
{
    internal static int instanceId = 1;
    private int _instanceId = 1;
    private asyncCallbackWindowEnumerate _windowEnum;

    public cWindowConnection(Control ControlWhichGetInvoked, ref cConnection Conn, ref HasEnumeratedEventHandler de) : base(ControlWhichGetInvoked, ref Conn)
    {
        instanceId += 1;
        _instanceId = instanceId;
        _windowEnum = new asyncCallbackWindowEnumerate(ref _control, de, ref this, _instanceId);
    }
    ...
    public delegate void HasEnumeratedEventHandler(bool Success, Dictionary<string, windowInfos> Dico, string errorMessage, int instanceId);
    ...
    // Enumerate threads
    public int Enumerate(bool getFixedInfos, ref int pid, bool unNamed, bool all, int forInstanceId = -1)
    {
        System.Threading.ThreadPool.QueueUserWorkItem(new System.Threading.WaitCallback(_windowEnum.Process()), new asyncCallbackWindowEnumerate.poolObj(pid, all, unNamed, forInstanceId));
    }
    ...
    protected new void _sock_ReceivedData(ref cSocketData data)
    {
        ...
        if (data.Type == cSocketData.DataType.RequestedList && data.Order == cSocketData.OrderType.RequestWindowList)
        {
            if (_instanceId == data.InstanceId)
                // OK it is for me
                _windowEnum.GotListFromSocket(ref data.GetList, ref data.GetKeys);
        }
    }

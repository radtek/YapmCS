...
public partial class privilegeList : customLV
{
    ...
    public privilegeList()
    {
        ...
        _privilegeConnection = new cPrivilegeConnection(this, ref _connectionObject, ref new cPrivilegeConnection.HasEnumeratedEventHandler(HasEnumeratedEventHandler));
        ...
        // Set handlers
        _privilegeConnection.Disconnected = new cPrivilegeConnection.DisconnectedEventHandler(HasDisconnected);
        _privilegeConnection.Connected = new cPrivilegeConnection.ConnectedEventHandler(HasConnected);
    }

    public event ItemAddedEventHandler ItemAdded;

    public delegate void ItemAddedEventHandler(ref cPrivilege item);

    public event ItemDeletedEventHandler ItemDeleted;

    public delegate void ItemDeletedEventHandler(ref cPrivilege item);
    ...
    private int _pid;
    private bool _first;
    private Dictionary<string, cPrivilege> _dicoNew = new Dictionary<string, cPrivilege>();
    private Dictionary<string, cPrivilege> _dicoDel = new Dictionary<string, cPrivilege>();
    private Dictionary<string, cPrivilege> _buffDico = new Dictionary<string, cPrivilege>();
    private Dictionary<string, cPrivilege> _dico = new Dictionary<string, cPrivilege>();
    ...
    private cPrivilegeConnection __privilegeConnection;

    private cPrivilegeConnection _privilegeConnection
    {
        [MethodImpl(MethodImplOptions.Synchronized)]
        get
        {
            return __privilegeConnection;
        }

        [MethodImpl(MethodImplOptions.Synchronized)]
        set
        {
            if (__privilegeConnection != null)
            {
            }

            __privilegeConnection = value;
            if (__privilegeConnection != null)
            {
            }
        }
    ...
    public override void UpdateItems()
    {
        ...
        if (_privilegeConnection.IsConnected)

            // Now enumerate items
            _privilegeConnection.Enumerate(_first, ref _pid);
    }

    // Get all items (associated to listviewitems)
    public Dictionary<string, cPrivilege>.ValueCollection GetAllItems()
    {
        return _dico.Values;
    }

    // Get the selected item
    public cPrivilege GetSelectedItem()
    {
    ...
    public cPrivilege GetItemByKey(string key)
    {
    ...
    // Get selected items
    public new Dictionary<string, cPrivilege>.ValueCollection GetSelectedItems()
    {
        Dictionary<string, cPrivilege> res = new Dictionary<string, cPrivilege>();
    ...
    private void HasEnumeratedEventHandler(bool Success, Dictionary<string, privilegeInfos> Dico, string errorMessage, int instanceId)
    {
        generalLvSemaphore.WaitOne();
        ...
        if (Success == false)
        {
            ...
            GotAnError?.Invoke("Privilege enumeration", errorMessage);
            generalLvSemaphore.Release();
            return;
        }
        ...
        foreach (cPrivilege z in _dico.Values)
        {
            if (z.IsKilledItem)
        ...
        foreach (System.Collections.Generic.KeyValuePair<string, privilegeInfos> pair in Dico)
        {
        ...
        {
            int x = 0;
            cPrivilege _item = _dico[it.Name];
    ...
    // Set backcolor
    private void SetItemBackColor(ref ListViewItem item)
    {
        switch (_dico[item.Name].Infos.Status)
        {
            ...
            case Native.Api.NativeEnums.SePrivilegeAttributes.EnabledByDefault:
            case Native.Api.NativeEnums.SePrivilegeAttributes.DisabledByDefault:
                {

...
public partial class processesInJobList : customLV
{
    ...
    public processesInJobList()
    {
        _connectionObject = new cConnection();
        _jobConnection = new cJobConnection(this, ref _connectionObject, ref new cJobConnection.HasEnumeratedProcInJobEventHandler(HasEnumeratedEventHandler));
        ...
        // Set handlers
        _jobConnection.Disconnected = new cJobConnection.DisconnectedEventHandler(HasDisconnected);
        _jobConnection.Connected = new cJobConnection.ConnectedEventHandler(HasConnected);
    }
    ...
    private cJobConnection __jobConnection;

    private cJobConnection _jobConnection
    {
        [MethodImpl(MethodImplOptions.Synchronized)]
        get
        {
            return __jobConnection;
        }

        [MethodImpl(MethodImplOptions.Synchronized)]
        set
        {
            if (__jobConnection != null)
            {
            }

            __jobConnection = value;
            if (__jobConnection != null)
            {
            }
        }
    ...
    private void HasEnumeratedEventHandler(bool Success, Dictionary<string, processInfos> Dico, string errorMessage, int instanceId)
    {
        ...
        foreach (processInfos cp in Dico.Values)
        {
            if (!(_dico.ContainsKey(cp.ProcessId.ToString())))
                // Add to dico

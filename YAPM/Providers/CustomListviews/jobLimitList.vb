...
Public Class jobLimitList
    Inherits customLV

    Public Event ItemAdded(ByRef item As cJobLimit)
    Public Event ItemDeleted(ByRef item As cJobLimit)
    ...
    Private _dicoNew As New Dictionary(Of String, cJobLimit)
    Private _dicoDel As New Dictionary(Of String, cJobLimit)
    Private _buffDico As New Dictionary(Of String, cJobLimit)
    Private _dico As New Dictionary(Of String, cJobLimit)
    Private WithEvents _connectionObject As New cConnection
    Private WithEvents _jobLimitConnection As New cJobLimitConnection(Me, _connectionObject, New cJobLimitConnection.HasEnumeratedEventHandler(AddressOf HasEnumeratedEventHandler))
...
#End Region
    ...
    Public Sub New()
        ...
        ' Set handlers
        _jobLimitConnection.Disconnected = New cJobLimitConnection.DisconnectedEventHandler(AddressOf HasDisconnected)
        _jobLimitConnection.Connected = New cJobLimitConnection.ConnectedEventHandler(AddressOf HasConnected)
    End Sub
    ...
    Public Overrides Sub UpdateItems()
        ...
        If _jobLimitConnection.IsConnected Then
    ...
    Public Function GetAllItems() As Dictionary(Of String, cJobLimit).ValueCollection
        Return _dico.Values
    End Function

    ' Get the selected item
    Public Function GetSelectedItem() As cJobLimit
    ...
    Public Function GetItemByKey(ByVal key As String) As cJobLimit
    ...
    ' Get selected items
    Public Shadows Function GetSelectedItems() As Dictionary(Of String, cJobLimit).ValueCollection
        Dim res As New Dictionary(Of String, cJobLimit)
    ...
    Private Sub HasEnumeratedEventHandler(ByVal Success As Boolean, ByVal Dico As Dictionary(Of String, jobLimitInfos), ByVal errorMessage As String, ByVal instanceId As Integer)

        generalLvSemaphore.WaitOne()

        If Success = False Then
            Trace.WriteLine("Cannot enumerate, an error was raised...")
            RaiseEvent GotAnError("Job limits connection enumeration", errorMessage)
            generalLvSemaphore.Release()
            Exit Sub
        End If
        ...
        For Each z As cJobLimit In _dico.Values
        ...
        For Each pair As System.Collections.Generic.KeyValuePair(Of String, jobLimitInfos) In Dico
        ...
        For Each z As String In _dicoNew.Keys
            Dim _it As cJobLimit = _dicoNew.Item(z)
        ...
        For Each z As String In _dicoNew.Keys
            ...
            For x As Integer = 1 To _subItems.Length - 1
                _subItems(x) = ""
            Next
            Dim _tmp As cJobLimit = _dico.Item(z)
    ...
    Private Shadows Function AddItemWithStyle(ByVal key As String, ByRef job As cJobLimit) As ListViewItem

...
public class cSearchConnection : cGeneralConnection
{
    internal static int instanceId = 1;
    private int _instanceId = 1;
    private asyncCallbackSearchEnumerate _searchEnum;

    public cSearchConnection(Control ControlWhichGetInvoked, ref cConnection Conn, ref HasEnumeratedEventHandler de) : base(ControlWhichGetInvoked, ref Conn)
    {
        instanceId += 1;
        _instanceId = instanceId;
        _searchEnum = new asyncCallbackSearchEnumerate(ref _control, de, ref this, _instanceId);
    }
    ...
    public delegate void HasEnumeratedEventHandler(bool Success, Dictionary<string, searchInfos> Dico, string errorMessage, int instanceId);
    ...
    // Enumerate services
    public int Enumerate(ref string str, bool caseS, Native.Api.Enums.GeneralObjectType include, int forInstanceId = -1)
    {
        System.Threading.ThreadPool.QueueUserWorkItem(new System.Threading.WaitCallback(_searchEnum.Process()), new asyncCallbackSearchEnumerate.poolObj(str, caseS, include, forInstanceId));
    }
    ...
    protected new void _sock_ReceivedData(ref cSocketData data)
    {
        ...
        if (data.Type == cSocketData.DataType.RequestedList && data.Order == cSocketData.OrderType.RequestSearchList)
        {
            if (_instanceId == data.InstanceId)
                // OK it is for me
                _searchEnum.GotListFromSocket(ref data.GetList, ref data.GetKeys);
        }
    }

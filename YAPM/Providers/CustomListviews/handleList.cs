...
public partial class handleList : customLV
{
    ...
    public handleList()
    {
        ...
        _handleConnection = new cHandleConnection(this, ref _connectionObject, ref new cHandleConnection.HasEnumeratedEventHandler(HasEnumeratedEventHandler));
        ...
        _IMG.Images.Add("key", My.Resources.Resources.key);
        _IMG.Images.Add("thread", My.Resources.Resources.thread);
        ...
        // Set handlers
        _handleConnection.Disconnected = new cHandleConnection.DisconnectedEventHandler(HasDisconnected);
        _handleConnection.Connected = new cHandleConnection.ConnectedEventHandler(HasConnected);
    }

    public event ItemAddedEventHandler ItemAdded;

    public delegate void ItemAddedEventHandler(ref cHandle item);

    public event ItemDeletedEventHandler ItemDeleted;

    public delegate void ItemDeletedEventHandler(ref cHandle item);
    ...
    // Private
    // ========================================
    private bool _unnamed = false;
    private int _pid;
    private bool _first;
    private Dictionary<string, cHandle> _dicoNew = new Dictionary<string, cHandle>();
    private Dictionary<string, cHandle> _dicoDel = new Dictionary<string, cHandle>();
    private Dictionary<string, cHandle> _buffDico = new Dictionary<string, cHandle>();
    private Dictionary<string, cHandle> _dico = new Dictionary<string, cHandle>();
    ...
    private cHandleConnection __handleConnection;

    private cHandleConnection _handleConnection
    {
        [MethodImpl(MethodImplOptions.Synchronized)]
        get
        {
            return __handleConnection;
        }

        [MethodImpl(MethodImplOptions.Synchronized)]
        set
        {
            if (__handleConnection != null)
            {
            }

            __handleConnection = value;
            if (__handleConnection != null)
            {
            }
        }
    ...
    public override void UpdateItems()
    {
        ...
        if (_handleConnection.IsConnected)

            // Now enumerate items
            _handleConnection.Enumerate(_first, ref _pid, _unnamed);
    }

    // Get all items (associated to listviewitems)
    public Dictionary<string, cHandle>.ValueCollection GetAllItems()
    {
        return _dico.Values;
    }

    // Get the selected item
    public cHandle GetSelectedItem()
    {
    ...
    public cHandle GetItemByKey(string key)
    {
    ...
    // Get selected items
    public new Dictionary<string, cHandle>.ValueCollection GetSelectedItems()
    {
        Dictionary<string, cHandle> res = new Dictionary<string, cHandle>();
    ...
    private void HasEnumeratedEventHandler(bool Success, Dictionary<string, handleInfos> Dico, string errorMessage, int instanceId)
    {
        generalLvSemaphore.WaitOne();
        ...
        if (Success == false)
        {
            ...
            GotAnError?.Invoke("Handle enumeration", errorMessage);
            generalLvSemaphore.Release();
            return;
        }
        ...
        foreach (cHandle z in _dico.Values)
        {
            if (z.IsKilledItem)
            {
                string _keyy = z.Infos.ProcessId.ToString() + "-" + z.Infos.Handle.ToString();
                _dicoDel.Add(_keyy, null);
            }
        }


        // Now add new items to dictionnary
        foreach (System.Collections.Generic.KeyValuePair<string, handleInfos> pair in Dico)
        {
            ...
            case "Directory":
                {
                    ...
                    string fName = _dico[key].Infos.Name;
                    if (System.IO.File.Exists(fName) | System.IO.Directory.Exists(fName))
                    {
                        System.Drawing.Icon img = Common.Misc.GetIcon2(fName, true);
                        if (img != null)
                        {
                            this.SmallImageList.Images.Add(fName, img);
                            item.ImageKey = fName;
                        }
                        else

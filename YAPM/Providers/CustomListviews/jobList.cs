...
public partial class jobList : customLV
{
    ...
    public jobList()
    {
        ...
        _jobConnection = new cJobConnection(this, ref _connectionObject, ref new cJobConnection.HasEnumeratedEventHandler(HasEnumeratedEventHandler));
    ...
    public delegate void ItemAddedEventHandler(ref cJob item);

    public event ItemDeletedEventHandler ItemDeleted;

    public delegate void ItemDeletedEventHandler(ref cJob item);
    ...
    // ========================================
    private bool _first;
    private Dictionary<string, cJob> _dicoNew = new Dictionary<string, cJob>();
    private Dictionary<string, cJob> _dicoDel = new Dictionary<string, cJob>();
    private Dictionary<string, cJob> _buffDico = new Dictionary<string, cJob>();
    private Dictionary<string, cJob> _dico = new Dictionary<string, cJob>();
    ...
    public Dictionary<string, cJob>.ValueCollection GetAllItems()
    {
        return _dico.Values;
    }

    // Get the selected item
    public cJob GetSelectedItem()
    {
    ...
    public cJob GetItemByKey(string key)
    {
    ...
    // Get selected items
    public new Dictionary<string, cJob>.ValueCollection GetSelectedItems()
    {
        Dictionary<string, cJob> res = new Dictionary<string, cJob>();
    ...
    private void HasEnumeratedEventHandler(bool Success, Dictionary<string, jobInfos> Dico, string errorMessage, int instanceId)
    {
        generalLvSemaphore.WaitOne();
        ...
        if (Success == false)
        {
            ...
            GotAnError?.Invoke("Job connection enumeration", errorMessage);
            generalLvSemaphore.Release();
            return;
        }
        ...
        foreach (cJob z in _dico.Values)
        {
            if (z.IsKilledItem)
        ...
        foreach (System.Collections.Generic.KeyValuePair<string, jobInfos> pair in Dico)
        {
    ...
    private new ListViewItem AddItemWithStyle(string key, ref cJob job)
    {

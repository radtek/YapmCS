...
public class cHeapConnection : cGeneralConnection
{
    internal static int instanceId = 1;
    private int _instanceId = 1;
    private asyncCallbackHeapEnumerate _heapEnum;

    public cHeapConnection(Control ControlWhichGetInvoked, ref cConnection Conn, ref HasEnumeratedEventHandler de) : base(ControlWhichGetInvoked, ref Conn)
    {
        instanceId += 1;
        _instanceId = instanceId;
        _heapEnum = new asyncCallbackHeapEnumerate(ref _control, de, ref this, _instanceId);
    }
    ...
    public delegate void HasEnumeratedEventHandler(bool Success, Dictionary<string, heapInfos> Dico, string errorMessage, int instanceId);
    ...
    // Enumerate threads
    public int Enumerate(ref int pid, int forInstanceId = -1)
    {
        System.Threading.ThreadPool.QueueUserWorkItem(new System.Threading.WaitCallback(_heapEnum.Process()), new asyncCallbackHeapEnumerate.poolObj(pid, forInstanceId));
    }
    ...
    protected new void _sock_ReceivedData(ref cSocketData data)
    {
        ...
        if (data.Type == cSocketData.DataType.RequestedList && data.Order == cSocketData.OrderType.RequestHeapList)
        {
            if (_instanceId == data.InstanceId)
                // OK it is for me
                _heapEnum.GotListFromSocket(ref data.GetList, ref data.GetKeys);
        }
    }
